...
API_URL = "https://trunghoang2002--qwen-audio-modal-serve-dev.modal.run/v1/chat/completions"
...
PAYLOAD_TEMPLATE = {
    "messages": [
        {"role": "system", "content": "You are a helpful assistant."},
        {
            "role": "user",
            "content": [
                {"type": "audio_url", "audio_url": {"url": None}},
                {"type": "text", "text": "Give me the transcription of the audio. Only provide the transcription without any additional information."}
            ]
        }
    ],
    "max_tokens": 500,
...
def get_payload(audio_url: str):
    ...
    payload["messages"][1]["content"][0]["audio_url"]["url"] = audio_url
    return payload
...
async def transcribe_file(
    ...
    try:
        ...
        for idx, chunk in enumerate(speech_chunks):
            ...
            chunk_line = result.get("choices", [{}])[0].get("message", {}).get("content", "").strip()
