...
from pyanote_diarization import RealtimeSpeakerDiarization
import torch
...
def get_speaker_id(audio_f32: np.ndarray, session_id: str = "default", max_speakers=None):
    ...
    with tempfile.NamedTemporaryFile(suffix=".wav") as tmp_file:
        ...
        sf.write(tmp_path, pad_audio_for_embedding(audio_f32), 16000, format="WAV")
        result = diarization_model(tmp_path, max_speakers=max_speakers, use_memory=True).serialize()['exclusive_diarization']
        speaker_ids = [item['speaker'] for item in result]
        return most_frequent(speaker_ids)
...
async def transcribe_file(
    ...
    try:
        ...
        if 'file_session_id' in locals():
            diarization_model.delete_session(file_session_id)
            logger.debug(f"[API] Cleaned up session: {file_session_id}")
        raise
