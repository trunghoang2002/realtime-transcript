...
class RealtimeSpeakerDiarization():
    """
    Realtime Speaker Diarization Pipeline with NeMo and persistent speaker embeddings
    
    This class supports:
    1. Persistent speaker tracking across multiple audio chunks using NeMo embeddings
    ...
    def __init__(self,
                 pretrained_speaker_model="titanet_large",  # ecapa_tdnn, speakerverification_speakernet
        ...
        print(f"🚀 Initializing NeMo Speaker Diarization Model...")
        print(f"   Device: {self.device}")
        print(f"   Speaker Model: {pretrained_speaker_model}")
        
        
        # Load NeMo model
        self.speaker_model = EncDecSpeakerLabelModel.from_pretrained(model_name=pretrained_speaker_model)
    ...
    def apply_realtime(self, file_or_audio_f32: Union[str, np.ndarray], 
        ...
        try:
            audio_length = audio_f32.shape[0]
            ...
            with torch.no_grad():
                logits, emb = self.speaker_model.forward(audio_signal, audio_signal_len)
...
if __name__ == "__main__":
    # Khởi tạo realtime pipeline với NeMo
    pipeline = RealtimeSpeakerDiarization(
